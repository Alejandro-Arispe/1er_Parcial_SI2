/** Envoltorio y paginacion de NestJS: { success, data, timestamp } y { data, meta }. */
...
/** Se desenvuelve una sola vez: una pagina tambien trae su propio campo data. */
export function desenvolverRespuesta<T>(respuesta: RespuestaBackend<T>): T {
  if (!respuesta || respuesta.success !== true || !('data' in respuesta)) {
    throw new ErrorApi('El servidor devolvio una respuesta incompatible.', 502);
  }
  return respuesta.data;
}

export function adaptarPagina<T, U>(pagina: PaginaBackend<T>, adaptar: (item: T) => U): Paginado<U> {
  return {
...
/** Solo para listas cortas o filtros que la API todavia no resuelve. */
...
export function paginarEnMemoria<T>(items: T[], { page = 1, page_size = 20 }: ParamsPaginacion): Paginado<T> {
  return {
